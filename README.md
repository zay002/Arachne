...
它会启动真机 bringup，等待 `/odom`、`/joint_states`、Aubo trajectory action 和夹具状态可用后打开示教面板；关闭面板时会自动停止 bringup。面板可以手动控制底盘、Aubo 末端和 MS42DC，记录当前底盘位姿、末端位置、关节角、夹具状态和等待步骤，并将 waypoint 保存到本地 `recordings/teach/` 后一键慢速回放。
