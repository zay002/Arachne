...
def control_owner_available(path: Path) -> tuple[bool, str]:
    if not path.exists():
    ...
    owner, pid = parse_control_owner(text)
    if pid is not None and not pid_alive(pid):
        try:
            path.unlink(missing_ok=True)
        except OSError as exc:
    ...
    pid_text = str(pid) if pid is not None else "unknown"
    return False, f"busy: {path} owner={owner or text.strip() or 'unknown'} pid={pid_text}"
