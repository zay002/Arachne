...
class JointStateMux(Node):
    def __init__(self) -> None:
        ...
        self.get_logger().info(
            ...
            f"{default_topic} + {gui_topic} + {gripper_topic} -> {output_topic}"
        )
    ...
    def _tick(self) -> None:
        if not self.default_positions and not self.gui_positions and not self.gripper_positions:
            return

        names = list(self.default_order)
        for order in (self.gui_order, self.gripper_order):
            for name in order:
