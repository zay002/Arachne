...
Third-party model and runtime assets are stored under `third_party/` and exposed to the ROS workspace through `src/vendor/` symlinks when they are ROS packages. Add future CAD, STL, SDK, or manual files only after checking their licenses.
...
- `third_party/ugv_sdk`: cloned from `agilexrobotics/ugv_sdk` at `c3dfaf444f9bae10757e546acae055aaf4a13de7`, used by `scout_base` for CAN communication.
...
- `third_party/ms42dc_step_motor_ros2`: extracted locally from the MS42DC vendor ROS2.zip by `scripts/prepare_ms42dc_ros2.sh`; provides `serial`, `step_motor`, and demo keyboard packages.
