...
- `src/arachne_operator/arachne_operator/teach_panel.py`: adds Aubo `Teach On/Off`, RX/RY/RZ wrist-orientation jogs, and `Duplicate` waypoint reuse.
- `src/arachne_operator/launch/teach_panel.launch.py`: exposes the teach command topic and orientation-jog parameters.
- `src/arachne_hardware/arachne_hardware/aubo_tcp_driver.py`: adds `aubo_teach_command_bridge`, which maps `/arachne/aubo/teach_command` to the official Aubo `jsonrpc_service` call `RobotManage.freedrive(true/false)`.
