...
`arachne_description` defines one complete `robot_description` for Scout 2.0, Aubo i5, MS42DC, mounting adapters, and optional sensor frames. Scout and Aubo are adapted from upstream open-source hardware descriptions; MS42DC uses the real local STEP model converted to STL for RViz.
...
The top-level Xacro includes each module, then connects them as one TF chain: Scout `base_link` to `arm_mount_link`, Aubo `aubo_base_link` to `tool0`, adapter to MS42DC, and MS42DC to `grasp_frame`. Control-specific files are optional and disabled by default so the first milestone stays visualization-focused.
