...
The remaining work is real-machine validation: Scout command-mode testing through the Waveshare USB-CAN-A adapter, MS42DC serial alias and safe travel calibration, Aubo firmware/SDK compatibility, and motion safety checks before enabling autonomous routines. WSL2 can be used for development and network control, but USB serial and USB-CAN devices must be explicitly passed through and verified before launch.
