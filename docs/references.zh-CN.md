...
第三方模型和运行时资产放在 `third_party/` 下；当它们是 ROS 包时，通过 `src/vendor/` 符号链接暴露给 workspace。后续添加 CAD、STL、SDK 或说明书文件时，需要先确认许可证、版本和来源。

## 下载的模型与运行时来源

- `third_party/aubo_description`：来自 `AuboRobot/aubo_description`，固定到 `47fa5e02fa873f27f7e812d31f31e3f4cf5e56b1`，包内声明 BSD。
...
- `third_party/ugv_sdk`：来自 `agilexrobotics/ugv_sdk`，固定到 `c3dfaf444f9bae10757e546acae055aaf4a13de7`，供 `scout_base` 进行 CAN 通信。
...
- `third_party/ms42dc_step_motor_ros2`：由 `scripts/prepare_ms42dc_ros2.sh` 从 MS42DC 厂家 ROS2.zip 本地解压，提供 `serial`、`step_motor` 和演示键盘包。
