...
第三方模型和运行时资产放在 `third_party/` 下；当它们是 ROS 包时，通过 `src/vendor/` 符号链接暴露给 workspace。仓库保留可复现运行需要的子集，包括 Aubo 官方 URDF/xacro 文本和 Aubo i5 系列 DAE/STL 网格；大型手册、视频、安装包和无关完整素材包由脚本或来源链接下载。

## 第三方运行时来源

- `third_party/aubo_description`：来自 `AuboRobot/aubo_description`，固定到 `47fa5e02fa873f27f7e812d31f31e3f4cf5e56b1`，包内声明 BSD；git 中保留官方 URDF/xacro 文件和 Aubo i5 系列 DAE/STL 运行网格，避免不同分支自行重写 Aubo 几何尺寸或关节定义。
