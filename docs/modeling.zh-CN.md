...
Arachne 暴露一个统一的 `robot_description`：Scout 底盘、Aubo 机械臂、可切换夹爪、末端支架和可选传感器都在同一棵 URDF 树里。MS42DC 和 AG95 两个版本唯一的区别，是 `ee_camera_support_link` 下方连接的夹爪不同。
...
```text
base_link -> aubo_base_link -> ... -> aubo_wrist3_Link -> ee_camera_support_link -> ms42dc_base_link -> grasp_frame
```

AG95 版本：

```text
base_link -> aubo_base_link -> ... -> aubo_wrist3_Link -> ee_camera_support_link -> ag95_base_link -> grasp_frame
```

`tool0` 仍然是 Aubo 法兰坐标，位于 `aubo_wrist3_Link` 下方；真实相机/夹具支架作为 `aubo_wrist3_Link` 的另一条分支建模。
