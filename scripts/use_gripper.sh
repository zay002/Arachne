...
usage() {
  cat <<'EOF'
Usage:
  ./scripts/use_gripper.sh <ms42dc|ag95> [mode] [extra launch args...]
...
Examples:
  ./scripts/use_gripper.sh ms42dc
  ./scripts/use_gripper.sh ag95 view
  ./scripts/use_gripper.sh ms42dc prehardware launch_rviz:=false
  ./scripts/use_gripper.sh ag95 translator input_topic:=/external/action_chunk
EOF
}
...
case "${MODE}" in
  ...
  gazebo|switch|demo)
    ...
    exec "${ROOT_DIR}/scripts/switch_demo.sh" "$@"
    ;;
