...
usage() {
  cat <<EOF
Usage:
  ./scripts/hardware/real_full_teach.sh --yes [-- teach_panel_launch_args...]
...
Examples:
  ...
  ./scripts/hardware/real_full_teach.sh --yes -- recording_dir:=recordings/teach
EOF
}
...
start_background "Scout + MS42DC bringup" \
  ...
  "${ROOT_DIR}/scripts/hardware/real_bringup.sh" --no-aubo
