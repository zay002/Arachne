...
if [[ "$(command -v python3 2>/dev/null || true)" != "/usr/bin/python3" ]]; then
  fail_or_warn "python3 resolves to $(command -v python3); run 'source scripts/env/arachne_env.sh' before ROS commands"
fi
...
if [[ -n "${ROS_DISTRO:-}" ]]; then
  ...
  fail_or_warn "ROS_DISTRO is not set; run source scripts/env/arachne_env.sh or source /opt/ros/humble/setup.bash"
fi
echo
...
do
  if [[ -e "${ROOT_DIR}/${path}" ]]; then
    ...
    warn "${path} not found; run ./scripts/hardware/prepare_real_hardware_ros.sh"
  fi
done
echo
