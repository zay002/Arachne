...
exec ros2 launch arachne_sensors gemini335.launch.py \
  ...
  with_tf:=true \
  camera_parent_frame:=tool0 \
  camera_optical_x:=-0.239469796 \
  camera_optical_y:=0.181459396 \
  camera_optical_z:=0.190102132 \
  camera_optical_roll:=0.083404947 \
  camera_optical_pitch:=-0.300045345 \
  camera_optical_yaw:=3.128380060 \
...
exec ros2 launch arachne_operator teach_panel.launch.py \
  ...
  camera_command:="ros2 launch arachne_sensors gemini335.launch.py publish_pointcloud:=false with_color_view:=false with_depth_view:=false with_tf:=true camera_parent_frame:=tool0 camera_optical_x:=-0.239469796 camera_optical_y:=0.181459396 camera_optical_z:=0.190102132 camera_optical_roll:=0.083404947 camera_optical_pitch:=-0.300045345 camera_optical_yaw:=3.128380060 projection_flip_x:=true projection_flip_y:=true color_yuv_layout:=YUYV color_width:=${CAMERA_COLOR_WIDTH} color_height:=${CAMERA_COLOR_HEIGHT} color_fps:=${CAMERA_COLOR_FPS} depth_width:=${CAMERA_DEPTH_WIDTH} depth_height:=${CAMERA_DEPTH_HEIGHT} depth_fps:=${CAMERA_DEPTH_FPS}" \
  ...
  cleanup_server_command:="scripts/vision/road_cleanup_task_server.sh patrol_pattern:=box_entry patrol_box_width_m:=1.0 patrol_box_height_m:=1.2 patrol_entry_m:=0.3 patrol_base_speed_mps:=0.06 max_round_trips:=2 base_step_timeout_sec:=32.0 detection_confidence:=0.35 loop:=true" \
