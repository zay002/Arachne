...
EE_SUPPORT_XYZ="${ARACHNE_GRASP_EE_SUPPORT_XYZ:-0.0 0.0 0.0}"
EE_SUPPORT_RPY="${ARACHNE_GRASP_EE_SUPPORT_RPY:-0.0 0.0 2.35619449019}"
TOOL_ADAPTER_XYZ="${ARACHNE_GRASP_TOOL_ADAPTER_XYZ:--0.049334103 0.049874070 0.021816675}"
TOOL_ADAPTER_RPY="${ARACHNE_GRASP_TOOL_ADAPTER_RPY:-1.570796327 0.0 0.0}"
...
REAL_JOINT_STATES_TOPIC="${ARACHNE_GRASP_REAL_JOINT_STATES_TOPIC:-/arachne/teach_visualization/joint_states}"
...
REAL_SEARCH_SCAN="${ARACHNE_GRASP_REAL_SEARCH_SCAN:-false}"
...
REAL_SEARCH_SCAN_LEFT_DELTA="${ARACHNE_GRASP_REAL_SEARCH_SCAN_LEFT_DELTA:-0.42,0.04,0.04,-0.06,-0.02,0.34}"
REAL_SEARCH_SCAN_RIGHT_DELTA="${ARACHNE_GRASP_REAL_SEARCH_SCAN_RIGHT_DELTA:--0.08,-0.02,-0.04,0.08,0.02,-0.10}"
PERCEPTION_ONLY_RESTART_SEC="${ARACHNE_GRASP_PERCEPTION_ONLY_RESTART_SEC:-1.0}"
...
AUBO_MOVE_JOINT_FALLBACK_INTERNAL="${ARACHNE_GRASP_AUBO_MOVE_JOINT_FALLBACK_INTERNAL:-false}"
AUBO_MOVE_JOINT_WAIT_SERVER_SEC="${ARACHNE_GRASP_AUBO_MOVE_JOINT_WAIT_SERVER_SEC:-0.5}"
REAL_START_JOINTS="${ARACHNE_GRASP_REAL_START_JOINTS:-${ARACHNE_GRASP_ARM_JOINTS:-}}"
...
REAL_FIXED_BASKET_JOINTS="${ARACHNE_GRASP_REAL_FIXED_BASKET_JOINTS:--1.187592607,0.410946348,2.275480439,0.311525006,1.571670234,0.382880501}"
...

{
  ...
  echo "real_search_scan: ${REAL_SEARCH_SCAN}"
  echo "real_search_scan_period_sec: ${REAL_SEARCH_SCAN_PERIOD_SEC}"
  echo "real_search_scan_left_delta: ${REAL_SEARCH_SCAN_LEFT_DELTA}"
  echo "real_search_scan_right_delta: ${REAL_SEARCH_SCAN_RIGHT_DELTA}"
...
run_pipeline() {
  ...
  local search_scan_args=(
    ...
    "--real-search-scan-left-delta=${REAL_SEARCH_SCAN_LEFT_DELTA}"
    "--real-search-scan-right-delta=${REAL_SEARCH_SCAN_RIGHT_DELTA}"
  ...
  if [[ "${EXECUTE_REAL}" == "true" ]]; then
    ...
    if [[ -n "${REAL_START_JOINTS}" ]]; then
      execute_args+=("--real-start-joints=${REAL_START_JOINTS}")
    fi
  ...
  "${ARACHNE_SYSTEM_PYTHON}" "${ROOT_DIR}/scripts/vision/grasp_preview_pipeline.py" \
    ...
    --perception-only-restart-sec "${PERCEPTION_ONLY_RESTART_SEC}" \
    "${projection_args[@]}" \
    "${search_scan_args[@]}" \
    "${execute_args[@]}" \
    "$@"
